...
/**
 * Generates random rhythm picked from scaleNotes rhythms units
 * @param vf
 * @param keySignature
 * @param scaleNotes
 * @param nbBars
...
export default function randomRhythmGenerator(
  ...
  if (scaleNotes.length === 1 && scaleNotes.includes("h")) {
    throw new Error(
      "scaleNotes must use at least use quarter notes ('q') or eighth notes ('2x8' or '8')"
    );
  }
  ...
  const concat = (a: StemmableNote[], b: StemmableNote[]): StemmableNote[] =>
    a.concat(b);
